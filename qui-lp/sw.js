...
function CreatePromiseData(timeout, opt) {
    ...
    var startTimer = function (timeout) {
        ...
        timer = setTimeout(function () { reject(new Error("The operation has timed out. timeoutValue  : ".concat(timeout))); }, timeout);
    };
    if (timeout) {
        startTimer(timeout);
    }
...
var waiters = new Map();
var clients = new Map();
selfSw.addEventListener("install", function (event) {
    selfSw.skipWaiting();
});
selfSw.addEventListener("online", function () {
    // console.log("online", version);
});
selfSw.addEventListener("offline", function () {
    // console.log("offline", version);
});
selfSw.addEventListener("activate", function (event) {
    // console.log("activate", version);
});
selfSw.addEventListener("message", function (event) {
    var pack = event.data;
    if (checkBinaryFile(pack.resourceType)) {
        var base64FileContent = getBase64FileContent(pack.resourceModel);
        var binResource = node_buffer_1.Buffer.from(base64FileContent, 'base64');
        var response = {
            resource: binResource,
            resourceId: pack.resourceId,
            resourceType: pack.resourceType
        };
        var waiter = waiters.get(pack.resourceId);
        waiter === null || waiter === void 0 ? void 0 : waiter.resolver(response);
    }
    else {
        var response = {
            resource: pack.resourceModel,
            resourceId: pack.resourceId,
            resourceType: pack.resourceType
        };
        var waiter = waiters.get(pack.resourceId);
...
var getBase64FileContent = function (fileContent) {
    return fileContent.split(',')[1];
};
var checkClient = function (clientId) {
    return clients.has(clientId);
};
function checkBinaryFile(extensionOfFile) {
    ...
    return __generator(this, function (_b) {
        switch (_b.label) {
            ...
            case 1:
                ...
                return [4 /*yield*/, imgPromData.promise];
            case 2:
                imgResp = _b.sent();
...
var isStartWithQuick = function (uri) {
    if (uri.startsWith("/quick/"))
        return true;
    return false;
};
var isStartWithTest = function (uri) {
    if (uri.startsWith("/test-qeditor-plateau-test/"))
        return true;
    return false;
};
...
var isStartWithLessSign = function (uri) {
...
var parseURI = function (uri) {
    ...
    var type = uri.slice(3, colonPos);
    var guid = uri.slice(colonPos + 1, uri.length - 2);
    return { type: type, guid: guid };
};
selfSw.addEventListener("fetch", function (event) {
    var resp = run(event);
    event.respondWith(resp);
    // event.respondWith(fetch(event.request));
});
var sendMessage = function (clientId, guid) { return __awaiter(void 0, void 0, void 0, function () {
    var clients;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0: return [4 /*yield*/, selfSw.clients.matchAll({ type: "window" })];
            case 1:
                clients = _a.sent();
                clients[0].postMessage(guid);
                return [2 /*return*/];
        }
    });
}); };
